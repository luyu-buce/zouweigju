...
# 修复PyInstaller打包后imageio/moviepy元数据丢失问题
...
def _patched_distribution(name):
    try:
        ...
        return type('_DummyDist', (), {
            'metadata': property(lambda _: {'Name': name, 'Version': '0.0.0'}),
            'version': '0.0.0',
            'read_text': lambda _, _2: '',
...
class StageAnimationTool:
    def __init__(self, root):
        ...
        self.element_groups = []  # 临时组合 [{'members': [item...], 'rotation': float}]
        self.keyframe_edit_item = None  # 组合状态下关键帧表格当前编辑的主元素
        self.multi_select_start_mouse_pos = None  # 多选拖动开始时的鼠标位置
        self.pending_deselect_item = None  # 待取消选中的对象（仅当未拖动时才取消）
        self.pending_deselect_group = None  # 待取消选中的组合
    ...
    def create_control_panel(self):
        ...
        self.grid_detail_expanded = tk.BooleanVar(value=False)
        grid_toggle_row = ttk.Frame(grid_frame)
        grid_toggle_row.pack(fill=tk.X, padx=2, pady=1)
        self.grid_switch = ttk.Checkbutton(grid_toggle_row, text="启用",
                                          variable=self.grid_enabled,
                                          command=self.update_stage_preview)
        self.grid_switch.pack(side=tk.LEFT, padx=(1, 5))
        self.grid_toggle_btn = ttk.Button(grid_toggle_row, text="▶ 展开辅助线设置",
                                          command=self.toggle_grid_detail_panel)
        self.grid_toggle_btn.pack(side=tk.LEFT, padx=1)

        self.grid_detail_panel = ttk.Frame(grid_frame)

        # 使用grid布局
        grid_g = ttk.Frame(self.grid_detail_panel)
        grid_g.pack(fill=tk.X, padx=2, pady=1)

        # X和Y间隔放在同一行
        ttk.Label(grid_g, text="X:").grid(row=0, column=0, sticky='e', padx=(0,1), pady=1)
        self.grid_x_entry = ttk.Entry(grid_g, width=7)
        self.grid_x_entry.insert(0, "5.0")
        self.grid_x_entry.grid(row=0, column=1, sticky='w', padx=0, pady=1)

        ttk.Label(grid_g, text="Y:").grid(row=0, column=2, sticky='e', padx=(1,1), pady=1)
        self.grid_y_entry = ttk.Entry(grid_g, width=7)
        self.grid_y_entry.insert(0, "5.0")
        self.grid_y_entry.grid(row=0, column=3, sticky='w', padx=0, pady=1)

        # 线形和线宽放在同一行
        ttk.Label(grid_g, text="线形:").grid(row=1, column=0, sticky='e', padx=(0,1), pady=1)
        ...
        self.grid_linestyle_combo = ttk.Combobox(grid_g, textvariable=self.grid_linestyle_var,
                                               values=["虚线", "实线", "点线", "点划线"],
                                               width=5, state="readonly")
        self.grid_linestyle_combo.grid(row=1, column=1, sticky='w', padx=0, pady=1)
        self.grid_linestyle_combo.bind('<<ComboboxSelected>>', self.on_grid_linestyle_change)

        ttk.Label(grid_g, text="宽:").grid(row=1, column=2, sticky='e', padx=(1,1), pady=1)
        ...
        self.grid_linewidth_combo = ttk.Combobox(grid_g, textvariable=self.grid_linewidth_var,
                                                ...
                                                width=5, state="readonly")
        self.grid_linewidth_combo.grid(row=1, column=3, sticky='w', padx=0, pady=1)
        self.grid_linewidth_combo.bind('<<ComboboxSelected>>', self.on_grid_linewidth_change)

        # 颜色单独一行
        ttk.Label(grid_g, text="色:").grid(row=2, column=0, sticky='e', padx=(0,1), pady=1)
        ...
        self.grid_color_combo = ttk.Combobox(grid_g, textvariable=self.grid_color_var,
                                           ...
                                           width=5, state="readonly")
        self.grid_color_combo.grid(row=2, column=1, sticky='w', padx=0, pady=1)
        self.grid_color_combo.bind('<<ComboboxSelected>>', self.on_grid_color_change)

        ttk.Button(grid_g, text="应用", width=10,
                  command=self.apply_grid_interval).grid(row=3, column=0, columnspan=4, sticky='ew', padx=1, pady=1)

        ttk.Separator(grid_g, orient='horizontal').grid(row=4, column=0, columnspan=4, sticky='ew', pady=(3, 2))
        ttk.Label(grid_g, text="自定:").grid(row=5, column=0, sticky='e', padx=(0,1), pady=1)
        ...
        self.custom_guide_axis_combo.grid(row=5, column=1, sticky='w', padx=0, pady=1)
        self.custom_guide_value_entry = ttk.Entry(grid_g, width=7)
        self.custom_guide_value_entry.grid(row=5, column=2, sticky='w', padx=0, pady=1)
        ttk.Button(grid_g, text="添加", width=5,
                  command=self.add_custom_guide).grid(row=5, column=3, sticky='w', padx=1, pady=1)

        self.custom_guides_listbox = tk.Listbox(grid_g, height=3, exportselection=False)
        self.custom_guides_listbox.grid(row=6, column=0, columnspan=4, sticky='ew', padx=1, pady=(1, 1))
        custom_guide_btns = ttk.Frame(grid_g)
        custom_guide_btns.grid(row=7, column=0, columnspan=4, sticky='ew', padx=1, pady=(0, 1))
        ...
        grid_g.columnconfigure(1, weight=0, minsize=0)
        grid_g.columnconfigure(2, weight=0, minsize=1)
        grid_g.columnconfigure(3, weight=0, minsize=0)
    ...
    def create_stage_preview(self):
        ...
        self.group_btn = ttk.Button(snap_frame, text="组合", command=self.group_selected_items)
        self.group_btn.pack(side=tk.LEFT, padx=5)

        self.ungroup_btn = ttk.Button(snap_frame, text="取消组合", command=self.ungroup_selected_items)
        self.ungroup_btn.pack(side=tk.LEFT, padx=5)
        ...
        self.insert_time_btn = ttk.Button(control_frame, text="插入时间", command=self.open_insert_time_dialog)
        self.insert_time_btn.pack(side=tk.LEFT, padx=5)
        self.delete_time_btn = ttk.Button(control_frame, text="删除时间", command=self.open_delete_time_dialog)
    ...
    def on_keyframe_list_select(self, event):
        ...
        selected_objects = [{'item': selected['item'], 'type': selected['type']} for selected in self.selected_items]
        first_item = self.selected_items[0]['item'] if self.selected_items else None
        first_item_type = self.selected_items[0]['type'] if self.selected_items else None

        active_group = self.get_active_selected_group()
        keyframe_display_item = first_item if len(selected_objects) == 1 else None
        if active_group and first_item is not None:
            keyframe_display_item = first_item
        ...
        if len(selected_objects) == 1 and first_item is not None and first_item_type is not None:
            ...
            if active_group and first_item is not None:
                self.current_item_label.config(text=f"组合已选中 {len(selected_objects)} 个对象，正在编辑: {first_item['name']}")
            else:
        ...
        self.keyframe_edit_item = keyframe_display_item

        # 单选或完整组合状态下，显示当前主元素的关键帧数据
        if keyframe_display_item is not None:
            # 添加关键帧数据
            for frame in sorted(keyframe_display_item["keyframes"]):
                pos = keyframe_display_item["positions"][frame]
    ...
    def update_keyframe_data(self):
        """更新关键帧数据"""
        current_item, _, _ = self.get_keyframe_edit_context()
        ...
        try:
            ...
            for item in self.keyframe_tree.get_children():
                ...
                x = float(values[1])
                y = float(values[2])
                old_pos = self.get_item_position_at_frame(current_item, frame)
                ...
                self.sync_group_keyframe_change(
                    current_item,
                    frame,
                    frame,
                    old_pos,
    ...
    def add_keyframe(self):
        """添加关键帧"""
        current_item, current_type, _ = self.get_keyframe_edit_context()
        if current_item is None:
            messagebox.showwarning("警告", "请先选择一个演员或道具")
            return
        ...
        try:
            ...
            def save_keyframe():
                try:
                    ...
                    old_base_pos = self.get_item_position_at_frame(current_item, frame)
                    old_rotation = self.get_item_rotation_at_frame(current_item, frame)
                    ...
                    # 更新旋转数据
                    if current_item in self.actors or current_item in self.props:
                        self.ensure_rotation_keyframe(current_item, frame, rotation)
                    ...
                    self.sync_group_keyframe_change(
                        ...
                        old_base_pos,
                        (x, y),
                        old_rotation if current_type in ('actor', 'prop') else None,
                        rotation if current_type in ('actor', 'prop') else None
                    )
                    ...
                    # 恢复舞台上的选中状态
                    self.select_items([selected['item'] for selected in saved_selected_items])
    ...
    def set_rotation_keyframe(self):
        ...
        if len(self.selected_items) > 1:
            active_group = self.get_active_selected_group()
            if not active_group:
                messagebox.showwarning("警告", "只能同时设置一个对象或一个组合的旋转角度")
                return
            try:
                rotation = float(self.rotation_angle_entry.get())
                current_rotation = active_group.get('rotation', 0.0)
                angle_delta = rotation - current_rotation
                members = active_group.get('members', [])
                self.save_state_to_history(f"设置组合旋转 ({len(members)}个元素 @ {self.current_second:.1f}秒)")
                self.rotate_group_items(active_group, angle_delta)
                active_group['rotation'] = rotation
                self.update_stage_preview()
                self.log(f"✓ 设置组合旋转: {rotation:.1f}°", 'success')
            except ValueError:
    ...
    def quick_rotate(self, angle_delta):
        ...
        if active_group:
            ...
            self.save_state_to_history(f"旋转组合 ({len(members)}个元素 {angle_delta:+.0f}°)")
            self.rotate_group_items(active_group, angle_delta)
            self.update_stage_preview()
            self.log(f"✓ 旋转组合: {names} {angle_delta:+.0f}°", 'success')
            return
    ...
    def delete_keyframe(self):
        ...
        if selected and keyframes_selected:
            ...
            if len(keyframes_selected) > 1:
                ...
                self.remove_group_keyframes(current_item, frames_to_delete)
                ...
                if frame in current_item["keyframes"]:
                    ...
                    return
                self.remove_group_keyframes(current_item, [frame])
    ...
    def on_keyframe_double_click(self, event):
        ...
        # 获取当前关键帧表格正在编辑的对象
        current_item, item_type, item_index = self.get_keyframe_edit_context()
        if current_item is None:
            messagebox.showwarning("警告", "请先选择一个演员或道具")
            return
        ...
        old_base_pos = self.get_item_position_at_frame(current_item, old_frame)

        # 获取旧的旋转角度
        old_rotation = 0.0
        if item_type in ('actor', 'prop') and "rotations" in current_item and old_frame < len(current_item["rotations"]):
        ...
        def save_changes():
            try:
                ...
                # 更新旋转角度
                if item_type in ('actor', 'prop'):
                    self.ensure_rotation_keyframe(current_item, new_frame, new_rotation)
                ...
                self.sync_group_keyframe_change(
                    ...
                    old_base_pos,
                    (new_x, new_y),
                    old_rotation if item_type in ('actor', 'prop') else None,
                    new_rotation if item_type in ('actor', 'prop') else None
                )
    ...
    def get_item_type_and_index(self, item):
        """获取元素类型和列表索引。"""
    ...
    def make_selected_entry(self, item):
        """把元素包装成selected_items使用的结构。"""
        item_type, item_index = self.get_item_type_and_index(item)
        ...
        return {
            ...
            'type': item_type,
            'index': item_index,
            'start_pos': self.get_item_current_position(item)
        }

    def prune_element_groups(self):
        """清理已删除元素造成的空组合。"""
        existing_items = set()
        for item in self.actors + self.props + self.textboxes:
            existing_items.add(id(item))

        valid_groups = []
        for group in self.element_groups:
            members = [item for item in group.get('members', []) if id(item) in existing_items]
            if len(members) >= 2:
                group['members'] = members
                valid_groups.append(group)
        self.element_groups = valid_groups

    def get_group_for_item(self, item):
        """获取元素所在的临时组合。"""
        self.prune_element_groups()
        for group in self.element_groups:
            if any(member is item for member in group.get('members', [])):
                return group
        return None

    def get_groups_for_selected_items(self):
        """获取当前选中元素涉及的所有组合。"""
        groups = []
        for selected in self.selected_items:
            group = self.get_group_for_item(selected['item'])
    ...
    def select_items(self, items):
        ...
        for item in items:
            if id(item) in seen:
                continue
            entry = self.make_selected_entry(item)
            if not entry:
                continue
            seen.add(id(item))
            self.selected_items.append(entry)
            list_index = entry['index']
            if entry['type'] == 'prop':
                list_index += len(self.actors)
            elif entry['type'] == 'textbox':
                list_index += len(self.actors) + len(self.props)
            self.keyframe_listbox.selection_set(list_index)

    def expand_selected_groups(self):
        """组合状态下，将选中项扩展为完整组合。"""
        expanded_items = [selected['item'] for selected in self.selected_items]
        changed = False

        for selected in list(self.selected_items):
            group = self.get_group_for_item(selected['item'])
            if not group:
                continue
            for member in group.get('members', []):
                if not any(member is item for item in expanded_items):
                    expanded_items.append(member)
                    changed = True

        if changed:
            self.select_items(expanded_items)

    def group_selected_items(self):
        """把当前选中的多个元素临时组合。"""
        if len(self.selected_items) < 2:
            self.log("⚠️ 请先选中至少两个元素再组合", 'warning')
            return

        members = []
        for selected in self.selected_items:
            item = selected['item']
            if not any(item is member for member in members):
                members.append(item)

        # 一个元素只能属于一个组合，新组合会覆盖旧组合关系。
        self.element_groups = [
            group for group in self.element_groups
            if not any(member is item for member in group.get('members', []) for item in members)
        ]
        self.element_groups.append({'members': members, 'rotation': 0.0})
        self.select_items(members)
        names = '、'.join(item['name'] for item in members)
        self.update_stage_preview()
        self.log(f"✓ 已组合 {len(members)} 个元素: {names}", 'success')

    def ungroup_selected_items(self):
        """取消当前选中元素所在的临时组合。"""
        groups = self.get_groups_for_selected_items()
        if not groups:
            self.log("⚠️ 当前选中元素不在组合中", 'warning')
            return

        self.element_groups = [group for group in self.element_groups if group not in groups]
        count = len(groups)
        self.update_stage_preview()
        self.log(f"✓ 已取消 {count} 个组合", 'success')

    def get_active_selected_group(self):
        """如果当前选择正好是一个完整组合，返回该组合。"""
        groups = self.get_groups_for_selected_items()
        if len(groups) != 1:
            return None
        group = groups[0]
        member_ids = {id(item) for item in group.get('members', [])}
        selected_ids = {id(selected['item']) for selected in self.selected_items}
        if member_ids and member_ids == selected_ids:
            return group
        return None

    def get_items_center(self, items):
        """计算一组元素当前位置的中心点。"""
        positions = [self.get_item_current_position(item) for item in items]
        if not positions:
            return (0, 0)
    ...
    def get_item_position_at_frame(self, item, frame):
        """获取元素在指定帧的位置。"""
        positions = item.get("positions", [])
        if not positions:
            return (0, 0)
        safe_frame = max(0, min(int(frame), len(positions) - 1))
        return positions[safe_frame]

    def get_item_rotation_at_frame(self, item, frame):
        """获取元素在指定帧的旋转角度。"""
        rotations = item.get("rotations", [])
        if not rotations:
            return 0.0
        safe_frame = max(0, min(int(frame), len(rotations) - 1))
        return rotations[safe_frame]

    def ensure_position_keyframe(self, item, frame, pos):
        """设置元素位置关键帧。"""
        if "positions" not in item:
            return
        frame = max(0, min(int(frame), self.total_frames - 1))
        item["positions"][frame] = pos
        if frame not in item["keyframes"]:
            item["keyframes"].append(frame)
            item["keyframes"].sort()

    def ensure_rotation_keyframe(self, item, frame, rotation):
        """设置元素旋转关键帧。"""
        if item not in self.actors and item not in self.props:
            return
        frame = max(0, min(int(frame), self.total_frames - 1))
        if "rotations" not in item:
            item["rotations"] = [0.0 for _ in range(self.total_frames)]
    ...
    def remove_keyframe_if_present(self, item, frame):
        """删除元素在指定帧的位置和旋转关键帧。"""
        frame = int(frame)
        if frame in item.get("keyframes", []):
            item["keyframes"].remove(frame)
        if frame in item.get("rotation_keyframes", []):
            item["rotation_keyframes"].remove(frame)

    def sync_group_keyframe_change(self, base_item, old_frame, new_frame, old_base_pos,
                                   new_base_pos, old_rotation=None, new_rotation=None):
        """组合内编辑一个成员关键帧时，按相对位置同步其他成员。"""
        group = self.get_active_selected_group()
        if not group or not any(member is base_item for member in group.get('members', [])):
            return

        old_frame = max(0, min(int(old_frame), self.total_frames - 1))
        new_frame = max(0, min(int(new_frame), self.total_frames - 1))
        rotation_delta = None
        if old_rotation is not None and new_rotation is not None:
            rotation_delta = new_rotation - old_rotation

        for member in group.get('members', []):
            if member is base_item:
                continue

            member_old_pos = self.get_item_position_at_frame(member, old_frame)
            offset_x = member_old_pos[0] - old_base_pos[0]
            offset_y = member_old_pos[1] - old_base_pos[1]
            member_new_pos = (new_base_pos[0] + offset_x, new_base_pos[1] + offset_y)

            if old_frame != new_frame:
                self.remove_keyframe_if_present(member, old_frame)
            self.ensure_position_keyframe(member, new_frame, member_new_pos)
            self.update_intermediate_frames(member)

            if rotation_delta is not None and (member in self.actors or member in self.props):
                member_old_rotation = self.get_item_rotation_at_frame(member, old_frame)
                if old_frame != new_frame and old_frame in member.get("rotation_keyframes", []):
                    member["rotation_keyframes"].remove(old_frame)
                self.ensure_rotation_keyframe(member, new_frame, member_old_rotation + rotation_delta)
                self.update_intermediate_rotations(member)

    def remove_group_keyframes(self, base_item, frames):
        """组合内删除一个成员关键帧时，同步删除其他成员同帧关键帧。"""
        group = self.get_active_selected_group()
        if not group or not any(member is base_item for member in group.get('members', [])):
            return
        ...
        for member in group.get('members', []):
            if member is base_item:
                continue
            for frame in frames:
                self.remove_keyframe_if_present(member, frame)
            self.update_intermediate_frames(member)
            if member in self.actors or member in self.props:
                self.update_intermediate_rotations(member)

    def get_listbox_item_by_index(self, index):
        """根据左侧列表索引获取元素。"""
        actor_count = len(self.actors)
        prop_count = len(self.props)
        if index < actor_count:
            return self.actors[index], 'actor', index
        if index < actor_count + prop_count:
            item_index = index - actor_count
            return self.props[item_index], 'prop', item_index
        textbox_index = index - actor_count - prop_count
        if 0 <= textbox_index < len(self.textboxes):
            return self.textboxes[textbox_index], 'textbox', textbox_index
        return None, None, None

    def get_keyframe_edit_context(self):
        """获取关键帧表格当前编辑的元素。"""
        if self.keyframe_edit_item is not None:
            item_type, item_index = self.get_item_type_and_index(self.keyframe_edit_item)
            if item_type is not None:
                return self.keyframe_edit_item, item_type, item_index

        selected = self.keyframe_listbox.curselection()
        if selected:
            return self.get_listbox_item_by_index(selected[0])
        if self.selected_items:
            selected_item = self.selected_items[0]
            return selected_item['item'], selected_item['type'], selected_item['index']
        return None, None, None

    def rotate_group_items(self, group, angle_delta):
        """把组合当成一个整体旋转。"""
        ...
        frame = self.current_frame
        center_x, center_y = self.get_items_center(members)
        radians = math.radians(-angle_delta)
        ...
        for item in members:
            ...
            item["positions"][frame] = new_pos
            if frame not in item["keyframes"]:
                item["keyframes"].append(frame)
                item["keyframes"].sort()
            self.update_intermediate_frames(item)

            if item in self.actors or item in self.props:
                if "rotations" not in item:
                    item["rotations"] = [0.0 for _ in range(self.total_frames)]
                if "rotation_keyframes" not in item:
                    item["rotation_keyframes"] = []
                item["rotations"][frame] = item["rotations"][frame] + angle_delta
                if frame not in item["rotation_keyframes"]:
                    item["rotation_keyframes"].append(frame)
                    item["rotation_keyframes"].sort()
                self.update_intermediate_rotations(item)

        group['rotation'] = group.get('rotation', 0.0) + angle_delta
    ...
    def on_mouse_press(self, event):
        ...
        if clicked_item is not None:
            clicked_group = self.get_group_for_item(clicked_item)
            clicked_group_was_all_selected = False
            if clicked_group:
                group_members = clicked_group.get('members', [])
                group_all_selected = all(
                    any(selected['item'] is member for selected in self.selected_items)
                    for member in group_members
                )
                clicked_group_was_all_selected = group_all_selected
                if ctrl_pressed:
                    if not group_all_selected:
                        current_items = [selected['item'] for selected in self.selected_items]
                        for member in group_members:
                            if not any(member is item for item in current_items):
                                entry = self.make_selected_entry(member)
                                if entry:
                                    self.selected_items.append(entry)
                                    current_items.append(member)
                        self.select_items(current_items)
                elif not group_all_selected:
                    ordered_members = [clicked_item] + [member for member in group_members if member is not clicked_item]
                    self.select_items(ordered_members)
            ...
            # Ctrl多选模式
            if ctrl_pressed:
                if clicked_group and clicked_group_was_all_selected:
                    self.pending_deselect_group = clicked_group
                    self.pending_deselect_item = clicked_item
                    print(f"🔶 Ctrl+点击已选中组合: {clicked_item['name']} (待确认：拖动or取消组合选择)")
                elif already_selected:
                    # 如果已选中，标记为待取消（在释放鼠标时根据是否拖动来决定是否取消）
                    # 这样可以避免在拖动多选对象时误取消
                    self.pending_deselect_item = clicked_item
                    self.pending_deselect_group = None
    ...
    def on_mouse_release(self, event):
        ...
        if drag_distance < self.drag_jitter_pixel_threshold:
            ...
            if self.pending_deselect_group is not None:
                group = self.pending_deselect_group
                group_members = group.get('members', [])
                self.selected_items = [
                    item for item in self.selected_items
                    if not any(item['item'] is member for member in group_members)
                ]
                self.keyframe_listbox.selection_clear(0, tk.END)
                for selected in self.selected_items:
                    list_index = selected['index']
                    if selected['type'] == 'prop':
                        ...
                        list_index += len(self.actors) + len(self.props)
                    self.keyframe_listbox.selection_set(list_index)
                names = '、'.join(member['name'] for member in group_members)
                print(f"❌ 取消选中组合: {names}")
                self.log(f"取消选中组合: {names}", 'info')
                self.pending_deselect_group = None
                self.pending_deselect_item = None
                self.update_stage_preview()
                pending_deselect_handled = True
            elif self.pending_deselect_item is not None:
                if len(self.selected_items) > 1:
                    # 不是最后一个对象，可以取消选中
    ...
    def toggle_grid_detail_panel(self):
        if self.grid_detail_expanded.get():
            self.grid_detail_panel.pack_forget()
            self.grid_toggle_btn.config(text="▶ 展开辅助线设置")
            self.grid_detail_expanded.set(False)
        else:
            self.grid_detail_panel.pack(fill=tk.X, padx=2, pady=2)
            self.grid_toggle_btn.config(text="▼ 收起辅助线设置")
            self.grid_detail_expanded.set(True)
        self.root.after(10, lambda: self._check_canvas_scroll_position())
