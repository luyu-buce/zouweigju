...
class StageAnimationTool:
    def __init__(self, root):
        ...
        self.root = root
        self.root.title("舞台走位动画制作工具 v2.7")
    ...
    def create_control_panel(self):
        ...
        stage_frame.grid(row=0, column=0, padx=(0, 2), pady=0, sticky=tk.N+tk.W+tk.E)
        
        ttk.Label(stage_frame, text="宽:").grid(row=0, column=0, padx=3, pady=2, sticky='e')
        self.width_entry = ttk.Entry(stage_frame, width=6)
        self.width_entry.insert(0, str(self.stage_width))
        self.width_entry.grid(row=0, column=1, padx=3, pady=2)
        
        ttk.Label(stage_frame, text="高:").grid(row=1, column=0, padx=3, pady=2, sticky='e')
        self.height_entry = ttk.Entry(stage_frame, width=6)
        self.height_entry.insert(0, str(self.stage_height))
        self.height_entry.grid(row=1, column=1, padx=3, pady=2)
        
        ttk.Button(stage_frame, text="✓应用", command=self.update_stage_size, width=8).grid(row=2, column=0, columnspan=2, pady=3)
        
        # 时间轴设置
        timeline_frame = ttk.LabelFrame(settings_container, text="时间轴设置")
        timeline_frame.grid(row=0, column=1, padx=(2, 0), pady=0, sticky=tk.N+tk.W+tk.E)
        
        ttk.Label(timeline_frame, text="秒数:").grid(row=0, column=0, padx=3, pady=2, sticky='e')
        self.seconds_entry = ttk.Entry(timeline_frame, width=6)
        self.seconds_entry.insert(0, str(self.total_seconds))
        self.seconds_entry.grid(row=0, column=1, padx=3, pady=2)
        
        ttk.Label(timeline_frame, text="速度:").grid(row=1, column=0, padx=3, pady=2, sticky='e')
        ...
        self.speed_combo.grid(row=1, column=1, padx=3, pady=2)
        self.speed_combo.bind('<<ComboboxSelected>>', self.on_speed_change)
        
        ttk.Button(timeline_frame, text="✓应用", command=self.update_timeline_settings, width=8).grid(row=2, column=0, columnspan=2, pady=3)
        ...
        name_frame.pack(fill=tk.X, padx=5, pady=3)
        
        ttk.Label(name_frame, text="名称:").pack(side=tk.LEFT, padx=2)
        self.actor_name_entry = ttk.Entry(name_frame, width=10)
        self.actor_name_entry.pack(side=tk.LEFT, padx=2, fill=tk.X, expand=True)
        ttk.Button(name_frame, text="✓", width=3, command=self.apply_actor_name).pack(side=tk.LEFT, padx=2)
        
        # 大小和字号在同一行
        size_font_frame = ttk.Frame(actor_frame)
        size_font_frame.pack(fill=tk.X, padx=5, pady=3)
        
        ttk.Label(size_font_frame, text="大小:").pack(side=tk.LEFT, padx=2)
        self.actor_size_entry = ttk.Entry(size_font_frame, width=6)
        self.actor_size_entry.insert(0, "1.0")
        self.actor_size_entry.pack(side=tk.LEFT, padx=2)
        
        ttk.Label(size_font_frame, text="字号:").pack(side=tk.LEFT, padx=(10, 2))
        self.actor_font_size = ttk.Entry(size_font_frame, width=6)
        ...
        # 不调用pack/grid方法，使控件不可见
        
        # 颜色选择
        color_frame = ttk.Frame(actor_frame)
        color_frame.pack(fill=tk.X, padx=5, pady=3)
        
        ttk.Label(color_frame, text="颜色:").pack(side=tk.LEFT, padx=2)
        self.actor_color_var = tk.StringVar(value="蓝色")
        self.actor_color_combo = ttk.Combobox(color_frame, textvariable=self.actor_color_var, 
                                            values=["蓝色", "红色", "绿色", "紫色", "橙色", "棕色"], 
                                            width=18, state="readonly")
        self.actor_color_combo.pack(side=tk.LEFT, padx=2, fill=tk.X, expand=True)
        
        # 操作按钮
        actor_btn_frame = ttk.Frame(actor_frame)
        actor_btn_frame.pack(fill=tk.X, padx=5, pady=(3, 5))
        ttk.Button(actor_btn_frame, text="➕ 添加", command=self.add_actor, width=15).pack(side=tk.LEFT, padx=2)
        ttk.Button(actor_btn_frame, text="🗑 删除", command=self.delete_actor, width=15).pack(side=tk.LEFT, padx=2)
        
        # 道具设置（优化版）
        prop_frame = ttk.LabelFrame(add_frame, text="🎭 道具设置")
        prop_frame.pack(fill=tk.X, padx=5, pady=5)
        self.prop_frame_ref = prop_frame  # 保存引用供后续使用
        
        # 名称输入（带应用按钮）
        prop_name_frame = ttk.Frame(prop_frame)
        prop_name_frame.pack(fill=tk.X, padx=5, pady=3)
        
        ttk.Label(prop_name_frame, text="名称:").pack(side=tk.LEFT, padx=2)
        self.prop_name_entry = ttk.Entry(prop_name_frame, width=10)
        self.prop_name_entry.pack(side=tk.LEFT, padx=2, fill=tk.X, expand=True)
        ttk.Button(prop_name_frame, text="✓", width=3, command=self.apply_prop_name).pack(side=tk.LEFT, padx=2)
        
        # 形状和字号在同一行
        shape_font_frame = ttk.Frame(prop_frame)
        shape_font_frame.pack(fill=tk.X, padx=5, pady=3)
        
        ttk.Label(shape_font_frame, text="形状:").pack(side=tk.LEFT, padx=2)
        self.prop_shape_var = tk.StringVar(value="矩形")
        self.prop_shape_combo = ttk.Combobox(shape_font_frame, textvariable=self.prop_shape_var, 
                                           values=["矩形", "圆形", "三角形"], 
                                           width=7, state="readonly")
        self.prop_shape_combo.pack(side=tk.LEFT, padx=2)
        self.prop_shape_combo.bind("<<ComboboxSelected>>", self.on_prop_shape_change)
        
        ttk.Label(shape_font_frame, text="字号:").pack(side=tk.LEFT, padx=(10, 2))
        self.prop_font_size = ttk.Entry(shape_font_frame, width=6)
        self.prop_font_size.insert(0, "10")
        self.prop_font_size.pack(side=tk.LEFT, padx=2)
        
        # 尺寸在同一行
        size_frame = ttk.Frame(prop_frame)
        size_frame.pack(fill=tk.X, padx=5, pady=3)
        
        self.prop_width_label = ttk.Label(size_frame, text="宽度:")
        self.prop_width_label.pack(side=tk.LEFT, padx=2)
        self.prop_width_entry = ttk.Entry(size_frame, width=6)
        self.prop_width_entry.insert(0, "1.0")
        self.prop_width_entry.pack(side=tk.LEFT, padx=2)
        
        self.prop_height_label = ttk.Label(size_frame, text="高度:")
        self.prop_height_label.pack(side=tk.LEFT, padx=(10, 2))
        self.prop_height_entry = ttk.Entry(size_frame, width=6)
        self.prop_height_entry.insert(0, "1.0")
        self.prop_height_entry.pack(side=tk.LEFT, padx=2)
        
        # 颜色选择
        color_frame = ttk.Frame(prop_frame)
        color_frame.pack(fill=tk.X, padx=5, pady=3)
        
        ttk.Label(color_frame, text="颜色:").pack(side=tk.LEFT, padx=2)
        self.prop_color_var = tk.StringVar(value="红色")
        self.prop_color_combo = ttk.Combobox(color_frame, textvariable=self.prop_color_var, 
                                           values=["红色", "蓝色", "绿色", "紫色", "橙色", "棕色"], 
                                           width=18, state="readonly")
        self.prop_color_combo.pack(side=tk.LEFT, padx=2, fill=tk.X, expand=True)
        
        # 操作按钮
        prop_btn_frame = ttk.Frame(prop_frame)
        prop_btn_frame.pack(fill=tk.X, padx=5, pady=(3, 5))
        ttk.Button(prop_btn_frame, text="➕ 添加", command=self.add_prop, width=15).pack(side=tk.LEFT, padx=2)
        ttk.Button(prop_btn_frame, text="🗑 删除", command=self.delete_prop, width=15).pack(side=tk.LEFT, padx=2)
        
        # 统一的名称样式编辑面板（用于演员和道具）
        unified_style_frame = ttk.LabelFrame(add_frame, text="🎨 样式编辑（演员/道具）")
        unified_style_frame.pack(fill=tk.X, padx=5, pady=5)
        
        ttk.Label(unified_style_frame, text="选中演员或道具后，编辑其名称样式", 
                 font=('Arial', 8), foreground='gray').pack(padx=5, pady=(3, 2))
        
        # 样式控件区域
        unified_controls = ttk.Frame(unified_style_frame)
        unified_controls.pack(fill=tk.X, padx=5, pady=3)
        
        # 字号
        ttk.Label(unified_controls, text="字号:").grid(row=0, column=0, padx=2, pady=2, sticky='e')
        self.unified_font_size = ttk.Entry(unified_controls, width=6)
        self.unified_font_size.insert(0, "12")
        self.unified_font_size.grid(row=0, column=1, padx=2, pady=2, sticky='w')
        
        # 颜色
        ttk.Label(unified_controls, text="颜色:").grid(row=0, column=2, padx=2, pady=2, sticky='e')
        self.unified_color_var = tk.StringVar(value="蓝色")
        self.unified_color_combo = ttk.Combobox(unified_controls, 
                                               textvariable=self.unified_color_var,
                                               values=["黑色", "白色", "蓝色", "红色", "绿色", "紫色", "橙色", "棕色"],
                                               width=7, state="readonly")
        self.unified_color_combo.grid(row=0, column=3, padx=2, pady=2, sticky='w')
        
        # 应用按钮（简化为两个）
        unified_btn_frame = ttk.Frame(unified_style_frame)
        unified_btn_frame.pack(fill=tk.X, padx=5, pady=(0, 5))
        
        ttk.Button(unified_btn_frame, text="应用字号", width=12,
                  command=self.apply_unified_font_size).pack(side=tk.LEFT, padx=2)
        ttk.Button(unified_btn_frame, text="应用颜色", width=12,
                  command=self.apply_unified_color).pack(side=tk.LEFT, padx=2)
        ...
        # 名称输入（带应用按钮）
        name_input_frame = ttk.Frame(textbox_frame)
        name_input_frame.pack(fill=tk.X, padx=5, pady=3)
        ttk.Label(name_input_frame, text="名称:").pack(side=tk.LEFT, padx=2)
        self.textbox_name_entry = ttk.Entry(name_input_frame, width=12)
        self.textbox_name_entry.pack(side=tk.LEFT, padx=2, fill=tk.X, expand=True)
        ttk.Button(name_input_frame, text="✓", width=3, command=self.update_textbox_name).pack(side=tk.LEFT, padx=2)
        
        # 内容输入（简洁布局）
        content_frame = ttk.Frame(textbox_frame)
        content_frame.pack(fill=tk.X, padx=5, pady=3)
        
        ttk.Label(content_frame, text="内容:").pack(side=tk.LEFT, padx=2, anchor='n')
        
        text_container = ttk.Frame(content_frame)
        text_container.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=2)
        
        self.textbox_content_entry = tk.Text(text_container, width=15, height=3, wrap=tk.WORD, font=('Microsoft YaHei', 9))
        ...
        ttk.Button(content_frame, text="✓", width=3, command=self.update_textbox_content).pack(side=tk.LEFT, padx=2, anchor='n')
        
        # 保存最后一次选中的文本和位置（用于部分样式编辑）
        self.last_text_selection = ""
        self.last_text_selection_range = None  # (start_index, end_index) 字符索引位置
        self.last_selected_textbox_for_ui = None  # 跟踪上一次更新UI的文本框，避免重复更新
        
        # 样式参数（优化布局）
        style_params_frame = ttk.Frame(textbox_frame)
        style_params_frame.pack(fill=tk.X, padx=5, pady=3)
        
        # 第一行：字号和颜色
        ttk.Label(style_params_frame, text="字号:").grid(row=0, column=0, padx=2, pady=2, sticky='e')
        self.textbox_font_size = ttk.Entry(style_params_frame, width=6)
        self.textbox_font_size.insert(0, "12")
        self.textbox_font_size.grid(row=0, column=1, padx=2, pady=2, sticky='w')
        
        ttk.Label(style_params_frame, text="颜色:").grid(row=0, column=2, padx=5, pady=2, sticky='e')
        self.textbox_color_var = tk.StringVar(value="黑色")
        self.textbox_color_combo = ttk.Combobox(style_params_frame, textvariable=self.textbox_color_var, 
                                           values=["黑色", "红色", "蓝色", "绿色", "紫色", "橙色", "白色"], 
                                           width=7, state="readonly")
        self.textbox_color_combo.grid(row=0, column=3, padx=2, pady=2, sticky='w')
        
        # 第二行：开始时间和结束时间
        ttk.Label(style_params_frame, text="开始:").grid(row=1, column=0, padx=2, pady=2, sticky='e')
        self.textbox_start_time = ttk.Entry(style_params_frame, width=6)
        self.textbox_start_time.insert(0, "0.0")
        self.textbox_start_time.grid(row=1, column=1, padx=2, pady=2, sticky='w')
        # 双击自动填充当前时间
        self.textbox_start_time.bind('<Double-Button-1>', lambda e: self._fill_current_time(self.textbox_start_time))
        
        ttk.Label(style_params_frame, text="结束:").grid(row=1, column=2, padx=5, pady=2, sticky='e')
        self.textbox_end_time = ttk.Entry(style_params_frame, width=6)
        self.textbox_end_time.insert(0, "5.0")
        self.textbox_end_time.grid(row=1, column=3, padx=2, pady=2, sticky='w')
        # 双击自动填充当前时间
        self.textbox_end_time.bind('<Double-Button-1>', lambda e: self._fill_current_time(self.textbox_end_time))
        
        # 第三行：选中文本显示
        ttk.Label(style_params_frame, text="选中:", font=('Arial', 8), foreground='gray').grid(row=2, column=0, padx=2, pady=2, sticky='e')
        self.selected_text_label = ttk.Label(style_params_frame, text="全部", 
                                             font=('Arial', 8), foreground='blue')
        self.selected_text_label.grid(row=2, column=1, columnspan=3, padx=2, pady=2, sticky='w')
        ...
        # 样式应用按钮（优化单行）
        style_apply_frame = ttk.Frame(textbox_frame)
        style_apply_frame.pack(fill=tk.X, padx=5, pady=3)
        
        ttk.Button(style_apply_frame, text="应用字号", width=10, 
                  command=self.apply_textbox_font_size).pack(side=tk.LEFT, padx=2)
        ttk.Button(style_apply_frame, text="应用颜色", width=10, 
                  command=self.apply_textbox_color).pack(side=tk.LEFT, padx=2)
        ttk.Button(style_apply_frame, text="应用时间", width=10, 
                  command=self.apply_textbox_time_range).pack(side=tk.LEFT, padx=2)
        ...
        textbox_btn_frame.pack(fill=tk.X, padx=5, pady=(3, 5))
        
        ttk.Button(textbox_btn_frame, text="➕ 添加", command=self.add_textbox, width=10).pack(side=tk.LEFT, padx=2)
        ttk.Button(textbox_btn_frame, text="🗑 删除", command=self.delete_textbox, width=10).pack(side=tk.LEFT, padx=2)
        ttk.Button(textbox_btn_frame, text="清除选择", width=10, command=clear_selection).pack(side=tk.LEFT, padx=2)
        ...
        # 第一行：开关和间隔
        grid_row1 = ttk.Frame(grid_frame)
        grid_row1.pack(fill=tk.X, padx=5, pady=3)
        
        self.grid_switch = ttk.Checkbutton(grid_row1, text="启用", 
                                          variable=self.grid_enabled,
                                          command=self.update_stage_preview)
        self.grid_switch.pack(side=tk.LEFT, padx=2)
        
        ttk.Label(grid_row1, text="X:").pack(side=tk.LEFT, padx=(10, 2))
        self.grid_x_entry = ttk.Entry(grid_row1, width=5)
        self.grid_x_entry.insert(0, "5.0")
        self.grid_x_entry.pack(side=tk.LEFT, padx=2)
        
        ttk.Label(grid_row1, text="Y:").pack(side=tk.LEFT, padx=(5, 2))
        self.grid_y_entry = ttk.Entry(grid_row1, width=5)
        self.grid_y_entry.insert(0, "5.0")
        self.grid_y_entry.pack(side=tk.LEFT, padx=2)
        
        ttk.Button(grid_row1, text="✓", width=3, 
                  command=self.apply_grid_interval).pack(side=tk.LEFT, padx=2)
        
        # 第二行：线形、线宽、颜色
        grid_row2 = ttk.Frame(grid_frame)
        grid_row2.pack(fill=tk.X, padx=5, pady=3)
        
        ttk.Label(grid_row2, text="线形:").pack(side=tk.LEFT, padx=2)
        self.grid_linestyle_var = tk.StringVar(value="虚线")
        self.grid_linestyle_combo = ttk.Combobox(grid_row2, textvariable=self.grid_linestyle_var,
                                               values=["虚线", "实线", "点线", "点划线"],
                                               width=6, state="readonly")
        self.grid_linestyle_combo.pack(side=tk.LEFT, padx=2)
        self.grid_linestyle_combo.bind('<<ComboboxSelected>>', self.on_grid_linestyle_change)
        
        ttk.Label(grid_row2, text="宽:").pack(side=tk.LEFT, padx=(5, 2))
        self.grid_linewidth_var = tk.StringVar(value="0.5")
        self.grid_linewidth_combo = ttk.Combobox(grid_row2, textvariable=self.grid_linewidth_var,
        ...
        ttk.Label(grid_row2, text="色:").pack(side=tk.LEFT, padx=(5, 2))
        self.grid_color_var = tk.StringVar(value="黑色")
        self.grid_color_combo = ttk.Combobox(grid_row2, textvariable=self.grid_color_var,
    ...
    def create_timeline(self):
        ...
        self.keyframe_listbox = tk.Listbox(list_frame, width=20, height=10, selectmode=tk.EXTENDED)
...
QQ：1248360754 小红书：5615193523"""
    ...
    def update_stage_preview(self):
        ...
        for actor in self.actors:
            ...
            linewidth = 3 if is_selected else 2  # 选中时线条加粗
            ...
            if actor["shape"] == "circle":
                ...
                # 使用带样式的名称渲染
                self.render_styled_name(self.ax, pos, actor["name"], font_size, color, char_styles, is_export=False)
        ...
        for prop in self.props:
            ...
            if prop["shape"] == "rectangle":
                ...
                if is_selected:
                    ...
                    self.ax.add_patch(highlight)
                # 使用带样式的名称渲染
                self.render_styled_name(self.ax, pos, prop["name"], font_size, color, char_styles, is_export=False)
    ...
    def render_styled_name(self, ax, pos, name, default_font_size, default_color, char_styles, is_export=False):
        """渲染带字符级样式的名称
        Args:
            ax: matplotlib轴对象
            pos: 位置 (x, y)
        ...
        for actor in self.actors:
            ...
            if self.is_position_in_visible_area(pos):
                if actor["shape"] == "circle":
                    ...
                    ax.add_patch(circle)
                    # 使用带样式的名称渲染
                    self.render_styled_name(ax, pos, actor["name"], font_size, color, char_styles, is_export)
                elif actor["shape"] == "square":
                    rect = Rectangle((pos[0]-actor["size"]/2, pos[1]-actor["size"]/2),
                                   actor["size"], actor["size"], 
                                   fill=False,  # 不填充
        ...
        for prop in self.props:
            ...
            if self.is_position_in_stage(pos):
                if prop["shape"] == "rectangle":
                    ...
                    ax.add_patch(rect)
                    # 使用带样式的名称渲染
                    self.render_styled_name(ax, pos, prop["name"], font_size, color, char_styles, is_export)
                elif prop["shape"] == "circle":
                    circle = Circle((pos[0], pos[1]), prop["width"]/2, 
                                 fill=False,  # 不填充
                                 color=color, 
