...
class StageAnimationTool:
    def __init__(self, root):
        ...
        self.root = root
        self.root.title("舞台走位动画制作工具 v2.4")
        ...
        # 显示欢迎消息
        self.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", 'info')
        self.log("舞台走位动画制作工具 v2.4", 'info')
        self.log("", 'info')
        self.log("快捷键: Ctrl+Z撤销 | Ctrl+Y重做 | 空格播放/暂停", 'info')
        self.log("多选: Ctrl+点击对象 | Ctrl+拖动框选", 'info')
        self.log("对齐: 智能吸附☑ | 对齐菜单(中心/左/右/上/下)", 'info')
        self.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", 'info')
    ...
    def create_control_panel(self):
        ...
        ttk.Button(actor_btn_frame, text="删除", command=self.delete_actor).pack(side=tk.LEFT, padx=2)
        ...
        ttk.Button(prop_btn_frame, text="删除", command=self.delete_prop).pack(side=tk.LEFT, padx=2)
...
QQ：1248360754 小红书：5615193523"""
    ...
    def update_stage_preview(self):
        ...
        for actor in self.actors:
            ...
            if actor["shape"] == "circle":
                ...
                # 如果被选中，添加外圈高亮
                if is_selected:
                    highlight = Circle((pos[0], pos[1]), actor["size"] * 1.15, 
    ...
    def is_point_in_item(self, x, y, item, item_type):
        ...
        if item_type == "actor":
            if item["shape"] == "circle":
                return ((x - pos[0])**2 + (y - pos[1])**2) <= item["size"]**2
    ...
    def on_mouse_release(self, event):
        ...
        # 增加调试信息
        if self.pending_deselect_item is not None:
            print(f"📏 拖动距离: {drag_distance:.3f}, 待取消对象: {self.pending_deselect_item['name']}")
        
        # 如果位置没有变化或移动距离很小（< 0.3，视为点击而非拖动）
        if drag_distance < 0.3:
        ...
        if self.pending_deselect_item is not None:
            print(f"🔄 发生了拖动(距离≥0.3)，取消待取消操作: {self.pending_deselect_item['name']}")
            self.pending_deselect_item = None
