...
class StageAnimationTool:
    def __init__(self, root):
        ...
        for buffer in buffer_sizes:
            try:
                ...
                print(f"✅ 音频系统初始化成功 (buffer={buffer})")
                initialized = True
                break
            except Exception as e:
                print(f"⚠️ buffer={buffer} 初始化失败: {e}")
                continue
        ...
        self.root = root
        self.root.title("舞台走位动画制作工具 v2.6")
    ...
    def create_control_panel(self):
        ...
        # 导出设置（独立显示）
        export_frame = ttk.LabelFrame(self.scrollable_frame, text="导出设置")
        export_frame.pack(fill=tk.X, padx=5, pady=5)
        
        ttk.Label(export_frame, text="导出帧率:").grid(row=0, column=0, padx=5, pady=2, sticky='e')
        self.export_fps_entry = ttk.Entry(export_frame, width=10)
        self.export_fps_entry.insert(0, "10")  # 默认导出帧率
        self.export_fps_entry.grid(row=0, column=1, padx=5, pady=2, sticky='w')
...
QQ：1248360754 小红书：5615193523"""
    ...
    def on_keyframe_list_select(self, event):
        ...
        if index < actor_count:
            ...
            self.actor_name_entry.insert(0, current_item['name'])
            ...
            self.prop_name_entry.insert(0, current_item['name'])
        elif index < actor_count + prop_count + len(self.textboxes):
            current_item = self.textboxes[index - actor_count - prop_count]
            self.current_item_label.config(text=f"当前编辑: 文本框 {current_item['name']}")
            is_textbox = True
            item_type = 'textbox'
            ...
            self.current_item_label.config(text=f"当前编辑: 旧版文本框")
        
        # 自动在舞台上选中该对象
        if item_type in ['actor', 'prop', 'textbox']:
            # 获取对象当前位置
            ...
            print(f"📋 从列表选中: {current_item['name']} ({item_type})")
        
        # 根据类型更新左侧面板的文本框编辑区域
        if is_textbox:
            # 更新左侧面板的文本框编辑区域内容
            self.textbox_name_entry.delete(0, tk.END)
            self.textbox_name_entry.insert(0, current_item.get('name', ''))
            
            # 显示当前帧的内容
            contents_array = current_item.get("contents", [])
            ...
            # 更新样式信息（如果有的话）
            styles = current_item.get("styles", {})
            ...
            print(f"左侧文本框编辑区域已更新，名称：{current_item.get('name', '')}, 当前帧内容：{current_frame_content}")
    ...
    def export_animation(self):
        ...
        try:
            ...
            # 计算后方备台区域高度以调整导出视图范围
            backstage_height = self.stage_height / 8
            # 与MP4导出保持一致，从-2开始以包含观众区域
            export_ax.set_ylim(-2, self.stage_height + backstage_height + 1)  # 包含观众区域和后方备台区域
            ...
            def is_position_in_stage(pos):
                ...
                # 检查Y轴范围：与视图范围完全一致（-2 到 backstage_height+1）
                # 包含观众区域、舞台、备台区域和文本框区域
            ...
            def update(frame):
                ...
                # 绘制新版文本框（支持多个独立文本框和富文本样式）
                # 使用统一的render_textboxes方法以支持字符级样式
                self.render_textboxes(export_ax, current_frame, is_export=True)
