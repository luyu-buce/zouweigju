...
try:
    ...
    try:
        # 兼容旧版本的 moviepy (1.x)
        from moviepy.editor import VideoFileClip, AudioFileClip, ImageSequenceClip  # type: ignore
    except ImportError as e:
        print(f"错误：无法导入 moviepy 库。请运行: pip install moviepy")
        raise e
import tempfile
import shutil
import time
...
def clip_with_duration(clip, duration):
    ...
    if hasattr(clip, 'with_duration'):
        ...
        raise AttributeError("Clip对象没有设置时长的方法")

def clip_with_audio(video_clip, audio_clip):
    """兼容新旧版本的设置音频方法"""
    if hasattr(video_clip, 'with_audio'):
        ...
        raise AttributeError("VideoClip对象没有设置音频的方法")

def clip_subclip(clip, start_time, end_time):
    """兼容新旧版本的剪辑方法"""
    if hasattr(clip, 'subclipped'):
        ...
        # 尝试使用 with_duration 作为备选方案（如果从0开始）
        if start_time == 0:
            duration = end_time - start_time
            return clip_with_duration(clip, duration)
        else:
            raise AttributeError(f"Clip对象没有剪辑方法，且不支持非零起始时间的剪辑")

def write_videofile_compat(clip, filename, **kwargs):
    """兼容新旧版本的视频导出方法"""
    import inspect
    
    # 获取 write_videofile 方法的参数列表
    write_method = clip.write_videofile
    ...
    # 打印被移除的参数（用于调试）
    if removed_params:
        print(f"⚠️ 以下参数在当前 moviepy 版本中不支持，已自动移除: {', '.join(removed_params)}")
    
    # 调用 write_videofile
    return write_method(filename, **filtered_kwargs)
...
class AnimationLoop:
    def __init__(self, app):
        ...
        self.target_fps = 15  # 目标刷新率（降低到15fps以最大化减少资源竞争）
        self.update_interval = 1000 // self.target_fps  # 更新间隔（67ms）
        self.audio_started = False  # 添加标志位，用于跟踪音频是否已开始播放
        self.audio_start_time = None  # 音频开始播放的时间戳
        self.ui_update_counter = 0  # UI更新计数器（用于跳帧）
    ...
    def stop(self):
        if self.running:
            ...
            if self.after_id:
                ...
                self.after_id = None
            # 暂停音频播放（使用淡出减少杂音）
            if self.app.audio_file and pygame.mixer.music.get_busy():
                try:
                    pygame.mixer.music.fadeout(50)  # 50ms快速淡出
                    pygame.time.wait(60)  # 等待淡出完成
                except:
                    pygame.mixer.music.pause()  # 如果淡出失败，直接暂停
            self.audio_started = False  # 重置音频播放状态
    ...
    def _update(self):
        ...
        # 音频同步处理 - 优化以减少杂音
        # 注意：为了减少杂音，禁用了自动重新同步功能
        # 只在首次播放时同步，之后让音频自然播放
        if self.app.audio_file:
            if not self.audio_started:
                # 首次启动音频
                pygame.mixer.music.play(start=self.app.current_second)
                self.audio_started = True
                self.audio_start_time = current_time
                print(f"🎵 音频开始播放，起始位置: {self.app.current_second:.2f}秒")
                print(f"💡 提示：已禁用自动音频同步以减少杂音")
            # 禁用定期同步检查以避免杂音
            # 如果需要监控偏差（仅用于调试），可以取消下面的注释
            # elif hasattr(self, 'last_sync_check') and (current_time - self.last_sync_check > 10000):
            #     audio_elapsed = (current_time - self.audio_start_time) / 1000.0
            #     expected_time = self.app.current_second - self.animation_start_second
            #     time_diff = abs(audio_elapsed - expected_time)
            #     print(f"📊 音频偏差监控: {time_diff:.2f}秒")
    ...
    def _update_ui(self):
        """更新UI显示（优化版本，减少不必要的操作）"""
        # 更新时间轴滑块（轻量级操作，每帧更新）
        self.app.is_time_scale_updating = True
        self.app.time_scale.set(self.app.current_second)
        self.app.is_time_scale_updating = False
        
        # 只在必要时更新文本框内容（减少频繁的delete/insert操作）
        if self.app.current_frame < len(self.app.text_box["contents"]) and self.app.current_frame >= 0:
            new_content = self.app.text_box["contents"][self.app.current_frame]
            if self.app.text_content_entry.get() != new_content:
                self.app.text_content_entry.delete(0, tk.END)
                self.app.text_content_entry.insert(0, new_content)
        
        # 只在秒数变化时更新时间显示
        new_second = str(int(self.app.current_second))
        if self.app.text_second_entry.get() != new_second:
            self.app.text_second_entry.delete(0, tk.END)
            self.app.text_second_entry.insert(0, new_second)
        
        # 清理临时关键帧（保持不变）
        self.app.cleanup_temp_keyframes_on_time_change()
        
        # 舞台预览更新优化：每2帧才更新一次（减少matplotlib绘图频率）
        self.ui_update_counter += 1
        if self.ui_update_counter >= 2:
            self.app.update_stage_preview()
            self.ui_update_counter = 0

class StageAnimationTool:
    def __init__(self, root):
        # 初始化pygame音频 - 使用优化的参数以减少杂音
        # 针对持续性"滋滋滋"杂音的激进优化方案：
        # frequency=22050: 降低采样率以减少数据量（从44100降低）
        # size=-16: 16位音频（-表示有符号）
        # channels=1: 单声道（降低50%数据量）
        # buffer=8192: 超大缓冲区（从4096增加到8192）
        # 这些设置会牺牲一些音质，但能大幅减少杂音
        try:
            pygame.mixer.quit()  # 先退出之前的初始化（如果有）
        except:
            pass
        pygame.mixer.init(frequency=22050, size=-16, channels=1, buffer=8192)
        print("🔊 音频初始化: 22050Hz, 单声道, 8192字节缓冲区（优化模式）")
        ...
        self.root = root
        self.root.title("舞台走位动画制作工具 v2.1")
    ...
    def on_closing(self):
        ...
        try:
            ...
            if self.audio_file and pygame.mixer.music.get_busy():
                pygame.mixer.music.fadeout(200)  # 200ms淡出
                pygame.time.wait(220)  # 等待淡出完成
    ...
    def import_audio(self):
        ...
        try:
            ...
            if self.audio_file and pygame.mixer.music.get_busy():
                try:
                    pygame.mixer.music.fadeout(100)  # 100ms淡出
                    pygame.time.wait(120)  # 等待淡出完成
                except:
                    pygame.mixer.music.stop()
            ...
            # 预加载音频：播放一小段然后立即停止，确保音频完全加载到缓冲区
            pygame.mixer.music.set_volume(0.0)  # 静音播放
            pygame.mixer.music.play(start=0)
            pygame.time.wait(100)  # 等待100ms确保加载
            pygame.mixer.music.stop()
            
            # 恢复音量并设置音频文件
            pygame.mixer.music.set_volume(self.audio_volume)
            self.audio_file = file_path
            print(f"✓ 音频文件已加载并预热: {os.path.basename(file_path)}")
            print(f"💡 音频设置: 22050Hz单声道, 8192字节缓冲区")
            ...
            # 显示成功消息
            messagebox.showinfo("成功", f"音频已导入，时长: {self.audio_duration:.2f}秒")
    ...
    def play_audio(self):
        """播放音频"""
        if self.audio_file:
            pygame.mixer.music.play(start=self.current_second)
    ...
    def remove_audio(self):
        ...
        if not self.audio_file:
            messagebox.showinfo("提示", "当前没有导入的音频文件")
            return
        ...
        try:
            ...
            # 显示成功消息
            messagebox.showinfo("成功", f"音频文件已删除\n\n已删除: {os.path.basename(old_audio_file)}\n时间轴设置保持不变")
    ...
    def create_timeline(self):
        ...
        # 作者信息文本
        author_info = """舞台走位动画制作工具 v2.1 测试版
由@天云 免费制作及分享
如有bug或好的优化建议，可联系：
QQ：1248360754 小红书：5615193523"""
    ...
    def on_keyframe_list_select(self, event):
        ...
        for frame in sorted(current_item["keyframes"]):
            ...
            self.keyframe_tree.insert('', 'end', values=(f"{seconds:.1f}秒", f"{pos[0]:.1f}", f"{pos[1]:.1f}"))
    ...
    def delete_keyframe(self):
        ...
        seconds = float(values[0].rstrip('秒'))  # 处理浮点数秒
        frame = int(seconds * self.fps)  # 计算对应的帧数
    ...
    def update_stage_preview(self):
        ...
        if self.is_playing and self.fixed_view_range:
            ...
            print(f"使用固定视图范围: X={xlim}, Y={ylim}")
            ...
            if waiting_actors or waiting_props or len(self.actors) > 0 or len(self.props) > 0:
                min_y -= 1.0
            self.ax.set_xlim(-self.stage_width/2 - invisible_width, self.stage_width/2 + invisible_width)
            # 计算后方备台区域高度以调整视图范围
            backstage_height = self.stage_height / 8
            self.ax.set_ylim(min_y, self.stage_height + backstage_height + 1)
        ...
        # 设置标题
        self.ax.set_title(f'当前时间: {self.current_second:.1f}秒', fontsize=12, pad=20)  # 增加标题上方间距
        ...
        # 隐藏Y<0和Y>stage_height区域的Y轴刻度
        yticks = self.ax.get_yticks()
        yticks = yticks[(yticks >= 0) & (yticks <= self.stage_height)]  # 只保留舞台范围内的刻度
        ...
        for line in self.ax.get_xgridlines():
            line.set_ydata([0, self.stage_height])
        for line in self.ax.get_ygridlines():
            line.set_xdata([-self.stage_width/2, self.stage_width/2])
        ...
        # 设置Y轴范围显示
        self.ax.spines['left'].set_bounds(0, self.stage_height)
        self.ax.spines['right'].set_bounds(0, self.stage_height)
    ...
    def insert_keyframe(self):
        ...
        if self.current_frame not in self.last_dragged_item["keyframes"]:
            ...
            # 显示成功提示
            messagebox.showinfo("成功", f"成功在时间点 {self.current_frame} 插入关键帧")
    ...
    def batch_insert_keyframe(self):
        ...
        if count > 0:
            messagebox.showinfo("批量插入完成", f"已为 {count} 个元素在第 {current_time} 秒插入关键帧")
        else:
            messagebox.showinfo("批量插入", f"所有元素在第 {current_time} 秒都已有关键帧")
    ...
    def export_animation(self):
        ...
        try:
            ...
            if progress_window is not None:
                ...
                # 显示成功消息
                messagebox.showinfo("成功", f"动画已导出到: {export_path}")
    ...
    def load_project(self):
        ...
        try:
            ...
            # 显示成功消息
            messagebox.showinfo("成功", f"项目已从 {file_path} 导入")
            ...
            try:
                ...
                try:
                    ...
                    if audio_clip.duration > video_clip.duration:
                        # 裁剪音频以匹配视频长度
                        audio_clip = clip_subclip(audio_clip, 0, video_clip.duration)
                    else:
                        # 延长视频以匹配音频长度
                        video_clip = clip_with_duration(video_clip, audio_clip.duration)
                    
                    # 将音频添加到视频
                    final_clip = clip_with_audio(video_clip, audio_clip)
                    ...
                    write_videofile_compat(
                        ...
                        audio_codec='aac',
                        fps=export_fps,
                        verbose=False,  # 旧版本参数，新版本会自动忽略
                        logger=None,    # 旧版本参数，新版本会自动忽略
                    ...
                    # 显示成功消息
                    messagebox.showinfo("成功", f"带音频的动画已导出到: {export_path}")
